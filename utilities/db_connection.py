...
class DbConnection:
    ...
    def get_db(self, string_connection):
        conn_pool = getattr(self.threadLocal, 'conn_pool', {})
        con = string_connection
        try:
            if con in conn_pool and conn_pool[con] is not None:
                ...
                r_conn = self.reset_db_conn(string_connection)
                return r_conn
    ...
    def reset_db_conn(self, string_connection):
        conn = None
        con_key = string_connection
        conn_pool = getattr(self.threadLocal, 'conn_pool', {})
        try:
        ...
        for i in range(3):
            try:
                conn = create_engine(string_connection)
                break
            except Exception as e:
                exc_obj = sys.exc_info()
                exc_type = exc_obj[0]
